...
Apollo is deployed to an AWS Lambda function as a stand-alone GraphQL server in production, but (invisibly) uses the popular [Express](https://expressjs.com/) Node.js application framework to power local development.
...
Resolvers are stored in the `api/src/graphql/resolvers` directory and use of the typed Prisma Client we generated often makes for simple functions.
