...
Let's have a look at this picture again. We can get into the ec2 instance. We are still in a publec subnet. We want to get into the private one that hosts the database. According to this pic, the bastion host communicataes to the rds with security groups.

In other words, the db will allow connections from a specific security group.

Security inbound (ingres) rules can allow traffic from an ip ar from another securtity group. In this sense, security group identifies the resource.
...
We allow connection on port 5432 (postgres) from instances that has the identity security group.

![problem](create_db_sc.png)

## attach the secuiryt groups

The identity sc needs to be attached to our bastion host (ec2 server). We already have one called ssh that is responsibel for the ssh connection. In the action bar we can change the securiy groups.


![problem](attach_new_sc_to_ec2.png)

We simply select to add another one so we will have two sc attached to ec2.

In rds we just need to modify our db instance.

![problem](attach_sc_to_db.png)

# connect with dbweave

We already know ssh work, we tried it. Know we need to use ssh to connect to a database.
...
We gave these credentails to aws during te db creation. The endpoint can be found in the db dashboard.

Now comes the ssh part.

![problem](dbw_ssh.png)

The host ip is the public ip of our ec2 instance. User in a case of an amazon AMI instance is ec2-user. Select public key as an authentication method. Then select the key file from your hard drive.

If everything is set up correctly then we should see the follwing window after clicking on test connection.


![problem](dbw_conn.png)
...
In this case im just using psql to connect to the db with the help of an ssh tunnel
...
The above command means that localhost:2222 should point to database-1.c2avit44uwcq.eu-west-1.rds.amazonaws.com:5432.

To estabslish the connection use bastion.pem private key and ssh into the machene that can be found at 3.248.195.32 with the user ec2-user.

Before initiateing this command psql fails of course.

![problem](psql_disconn.png)

But after hitting enter psql connect to our rds instance that has no public ip.

![problem](psql_success.png)

That is how we can use a db locally with the help of a bastion server.
