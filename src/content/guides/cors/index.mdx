...
Things may seem okay when developing those parts independently, but when brought together there may be errors relating to CORS. In this guide and [another](/guides/cors-aws-lambda), we’ll look at how to avoid common pitfalls—first learning about CORS generally, and then applied in the context of a Serverless environment on AWS Lambda as recommended for Saruni.
...
All the code shared below can be found in [this Git repository](https://github.com/tambium/GUIDE_RESOURCE_cors-raw).
