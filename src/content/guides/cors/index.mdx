...
This error looks similar to the last but now makes reference to a preflight request. Preflight requests are triggered when a set of request conditions—best outlined in [this article](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS#Examples_of_access_control_scenarios), or more concretely in the [Fetch spec](https://fetch.spec.whatwg.org/)—are not met.
